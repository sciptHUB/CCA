---
title: "Prepare data in GCT file format for GenePattern"
output: html_notebook
---

Download data.
```{r}
myPath <- 'C:/Users/patipark/Dropbox/CCA/SubClassMapping'
setwd(myPath)

Label_TCGA <- xlsx::read.xlsx('ClassLabel_A_TCGA.xlsx',sheetIndex = 1)

Label_SISP <- xlsx::read.xlsx('ClassLabel_B_SiSP.xlsx',sheetIndex = 1)

Matrix_TCGA <- xlsx::read.xlsx('DataSet_A_TCGA.xlsx',sheetIndex = 1)

Matrix_SISP <- xlsx::read.xlsx('DataSet_B_SiSP.xlsx',sheetIndex = 1)

# change column names for label tables
colnames(Label_TCGA) <- c('Name','Group.name','Group.number')

colnames(Label_SISP) <- c('Name','Group.name','Group.number')

```

Extract common genes.
```{r}
# find common genes between the two data sets
genes <- intersect(Matrix_TCGA$Gene,Matrix_SISP$Gene)

```


Prepare expression data in txt format for ExpressionSet object building
Prepare CLS files for CCA and ICC comparison
```{r}

# make the format to fit with txt file
txtformat.TCGA <- data.frame('id' = Label_TCGA$Name,
                            'subgroup' = Label_TCGA$Group.number
                            )


# make the format to fit with txt file
txtformat.SISP <- data.frame('id' = Label_SISP$Name,
                            'subgroup' = Label_SISP$Group.number
                            )
```

Save Label data in txt format.
```{r}
setwd(myPath)
write.table(txtformat.TCGA,'CLS_TCGA.txt',quote = FALSE,row.names = FALSE,
             col.names = TRUE,sep = ',',eol = '\n')

write.table(txtformat.SISP,'CLS_SISP.txt',quote = FALSE,row.names = FALSE,
            col.names = TRUE,sep = ',', eol = '\n')
```

Prepare expression matrix.
```{r}
# biomarkers <- cca.genes$Gene
Matrix_TCGA <- Matrix_TCGA %>%
  tibble::column_to_rownames(., var = 'Gene')

Matrix_SISP <- Matrix_SISP %>%
  tibble::column_to_rownames(., var = 'Gene')

```

Save expression matrix as txt files (for building GCT files later).
```{r}
setwd(myPath)

write.table(Matrix_TCGA,'GCT_TCGA.txt',quote = FALSE,row.names = TRUE,
            col.names = TRUE,sep = ' ',eol = '\n')

write.table(Matrix_SISP,'GCT_SISP.txt',quote = FALSE,row.names = TRUE,
            col.names = TRUE,sep = ' ',eol = '\n')

```

Build eset data for CCA
```{r}
# download CLS.txt and GCT.txt data
setwd(myPath)
# phenoData:
tmp.cls <- read.table('CLS_TCGA.txt',sep = ",", header = TRUE) %>% as.data.frame()

# expression data:
tmp.expr <- read.table('GCT_TCGA.txt',sep = " ") %>% as.data.frame()

```

Check if samples in CLS and expression matrix were equal.
```{r}
# because number of samples in CLS and GCT matrix were not equal
common_id <- intersect(colnames(tmp.expr), tmp.cls$id)

# subset common id from CLS table
tmp.cls <- tmp.cls %>% 
  subset(id %in% common_id) %>% 
  tibble::column_to_rownames(., var = 'id')

tmp.expr <- tmp.expr[,common_id]

# create metadata 
metaData <-
  data.frame(labelDescription = c('subgroup'))

pdata <- Biobase::AnnotatedDataFrame(data = tmp.cls,varMetadata = metaData)

# create expression matrix
m <- as.matrix(tmp.expr)
...
```{r}
# check
identical(rownames(pdata),colnames(m))
## create ExpressionSet object:
eset <- new("ExpressionSet", exprs = m, phenoData = pdata)
```

Save GCT and CLS files
```{r}
dir.create(file.path(myPath, "TCGA"))
setwd(file.path(myPath,"TCGA"))

fpath <- getwd()

ArrayTools::createGSEAFiles(mydir = fpath,eSet = eset,catVar = "subgroup")
```

Build eset data from SISP data sets.
```{r}
setwd(myPath)

# phenoData:
tmp.cls <- read.table('CLS_SISP.txt',sep = ",", header = TRUE) %>% as.data.frame()

# expression data:
tmp.expr <- read.table('GCT_SISP.txt',sep = " ") %>% as.data.frame()
...
Save GCT and CLS files
```{r}
dir.create(file.path(myPath, "SISP"))
setwd(file.path(myPath,"SISP"))
...
Assign file location to variables.
```{r}
fpath.A <- file.path(myPath,'TCGA/Result/Pathway_Analysis/GSEA')

input.data.A <- file.path(fpath.A,'subgroup.probe.gct')
input.cls.A <- file.path(fpath.A,'subgroup.phenotype.cls')

fpath.B <- file.path(myPath,'SISP/Result/Pathway_Analysis/GSEA')

input.data.B <- file.path(fpath.B,'subgroup.probe.gct')
input.cls.B <- file.path(fpath.B,'subgroup.phenotype.cls')

```

Submap function (from Hoshida et al, 2007).
```{r}
submap.main <- function(
  ...
  ntag=100, #no. of marker genes
  nperm=50, #no. of random permutations for ES
  nperm.fisher=1000, #no. of random perm for Fisher's statistics
  weighted.score.type=1, #weight enrichment (signal-to-noise ratio)
  null.dist="pool", #Null distribution method. 'pool','each'
  p.corr="FDR", #p-value correction method. 'FDR','Bonferroni'
  clust.row=1,
  clust.col=1,
  nom.p.mat="F",
  create.legend="T",
...
)
{
    ...
    # if intersected number is less than predifined gene markers, it stops
    ...
    if (p.corr=="Bonferroni")
    {
        ...
        heatmap(bonf.SA.matrix,
                Rowv=clust.row,
                Colv=clust.col,
                scale="none",
                col=rainbow(1000,start=(0+.7*min.matrix),end=.7*max.matrix)) # omit 'gamma = 1'
    ...
    if (nom.p.mat=="T")
    {
        ...
        heatmap(nom.p.matrix.1[,,1],
                Rowv=NA,Colv=NA,
                scale="none",
                col=rainbow(1000,start=(0+.7*min(nom.p.matrix.1[,,1])),end=.7*max(nom.p.matrix.1[,,1]))) # omit 'gamma = 1'
        ...
        heatmap(t.nom.p.matrix.2,Rowv=NA,Colv=NA,scale="none",col=rainbow(1000,start=(0+.7*min(t.nom.p.matrix.2)),end=.7*max(t.nom.p.matrix.2))) # omit 'gamma = 1'
...
# take intersect genes in data.A and data.B
...
dir.create(file.path(myPath, "Results"))
setwd(file.path(myPath,"Results"))

submap.main(input.data.A = input.data.A,
            input.data.B = input.data.B,
            input.cls.A = input.cls.A,
            input.cls.B = input.cls.B,
            output.filename = 'TCGA_vs_SISP_600genes',
            ntag = 600, #no. of marker genes
            nperm = 100, #no. of random permutations for ES
            nperm.fisher = 1000, #no. of random perm for Fisher's statistics
            weighted.score.type = 1, #weight enrichment (signal-to-noise ratio)
            null.dist = "pool", #Null distribution method. 'pool','each'
            p.corr = "Bonferroni", #p-value correction method. 'FDR','Bonferroni'
            clust.row = 1,
            clust.col = 1,
            nom.p.mat = "T",
            create.legend = "T",
