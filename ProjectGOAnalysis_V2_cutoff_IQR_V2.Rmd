...
QT50.mat <- apply(df.mat[,-c(1)],1, function(i) {quantile(i,0.5)})
QT50.df <- data.frame('QT50' = QT50.mat)

df.mat2 <- cbind(df.mat[,1],QT50.df,df.mat[,c(2:16)])
# remove any gene with QT50 < 0
df.mat3 <- df.mat2[df.mat2$QT50 > 0,] 
colnames(df.mat3)[1] <- 'Gene.name'
```


```{r}
# normalize log2 with group median value
normIQR <- function(D) {
  d <- as.numeric(D)
...
df.normIQR <- t(apply(df.mat3[,3:17], 1, normIQR))
df.normIQR[!is.finite(df.normIQR)] <- 0
colnames(df.normIQR) <- colnames(df.mat3[,3:17])
# data as fold-change of expression to group median
df.norm <- log2(df.normIQR)
df.norm[!is.finite(df.norm)] <- 0
# This matrix will be used for GSEA evaluation
df.new <- cbind(data.frame('Gene.name' = df.mat3[,1]),as.data.frame(df.normIQR))

#############################################
# This section will calculate ~delta-delta-CT like comparing between test and control group
cell.CCA1 <- DRGROUP1 # MEK, Src-Abl sensitive
cell.CCA2 <- DRGROUP2 # MEK, Src-Abl insensitive

# separate CCA subtype
gr1.loc <- which(colnames(df.new) %in% cell.CCA1)
gr2.loc <- which(colnames(df.new) %in% cell.CCA2)
# calculate median of each subtype
gr1.median <- apply(df.new[,gr1.loc], 1, median)
gr2.median <- apply(df.new[,gr2.loc], 1, median)
# subtract log2(fold-change) with median of control subtype
gr.control <- gr2.median
df.control <- df.new[,gr2.loc] - gr.control
df.test <- df.new[,gr1.loc] - gr.control

df.all <- cbind(data.frame('Gene.name' = df.new[,1], df.test, df.control))
```
...
# create file for GSEA
df.B <- addENTREZID(df.new)
colnames(df.B)[c(1,2)] <- c('NAME','DESCRIPTION')
# save file
#write.table(df.B, file = 'GSEA_Matrix_Broad.txt', row.names = FALSE, quote = FALSE, sep = "\t")

df.C <- addENTREZID(df.all)
colnames(df.C)[c(1,2)] <- c('NAME','DESCRIPTION')
# save file
write.table(df.C, file = 'GSEA_Matrix_Broad_testcontrol.txt', row.names = FALSE, quote = FALSE, sep = "\t")
```


```{r}
# use MSigDB as gene sets 
# set reference file
library(GSEABase)
data.path <- 'C:/Users/patipark/Dropbox/CCA project/CCA RNA-seq'
data.folder <- 'msigdb_v6.1_files_local/msigdb_v6.1_GMTs'
data.list <- list.files(file.path(data.path,data.folder), pattern = '\\.entrez.gmt$')

gmtfile.hm <- file.path(data.path,data.folder,data.list[20])
c1 <- read.gmt(gmtfile.hm)

gmtfile.bp <- file.path(data.path,data.folder,data.list[15])
c2 <- read.gmt(gmtfile.bp)

gmtfile.kg <- file.path(data.path,data.folder,data.list[5])
c3 <- read.gmt(gmtfile.kg)

gmtfile.cp <- file.path(data.path,data.folder,data.list[7])
c4 <- read.gmt(gmtfile.cp)

gmtfile.os <- file.path(data.path,data.folder,data.list[18])
c5 <- read.gmt(gmtfile.os)
```


```{r}
myenrich <- function(df,col.ENTREZID,col.cell,db,f.switch){
...
```{r}
# save GSEA matrix
write.csv(Testmat.HM, file = 'GSEA_matrix_hallmark.csv',row.names = FALSE, quote = FALSE)
write.csv(Testmat.BP, file = 'GSEA_matrix_bp.csv', row.names = FALSE, quote = FALSE)
write.csv(Testmat.KG, file = 'GSEA_matrix_kegg.csv', row.names = FALSE, quote = FALSE)
write.csv(Testmat.CP, file = 'GSEA_matrix_cp.csv', row.names = FALSE, quote = FALSE)
write.csv(Testmat.OS, file = 'GSEA_matrix_os.csv', row.names = FALSE, quote = FALSE)
```


```{r}
# filter Hallmarks NES matrix extracting FDR qvalue < 0.25
sel25 <- Testmat.HM %>% filter(qvalues < 0.25)
sel25.all <- mutate(sel25, Subtype = ifelse(sel25$Cell %in% c(DRGROUP1), 1, 2))

Sig <- sel25.all %>% select_('ID','Cell','Subtype','NES','p.adjust','qvalues')
# convert p-value; smaller the larger
Sig$log2.p.adjust <- -log2(Sig$p.adjust)
Sig$Cell <- factor(Sig$Cell)
Sig$Subtype <- factor(Sig$Subtype)
...
ct_fig1 <- ggplot(Sig, aes(x = NES, # GR column on x-axis
                          y = reorder(ID,log2.p.adjust),
                          group = Cell,
                          shape = Cell)) + # Count as size of dot
  scale_shape_manual(values = 1:nlevels(Sig$Cell)) +
  geom_vline(xintercept = 0, 
             linetype = 'dotted', 
             color = 'red',
             size = 1) +
  geom_point(aes(color = factor(Subtype), size = log2.p.adjust)) +
  ...
  labs(    x = expression(NES),
           y = expression(Oncogenic~Signatures),
           size = expression(-log[2]~(Adj.p-value)),
           color = expression(Subtype),
           shape = expression(Cell))

ct_fig2 <- ggplot(Sig, aes(x = NES, # GR column on x-axis
                          y = reorder(ID,log2.p.adjust),
                          group = Subtype,
                          color = factor(Subtype))) + # Count as size of dot
  geom_vline(xintercept = 0, 
             linetype = 'dotted', 
             color = 'red',
             size = 1) +
  geom_point(aes(fill = factor(Subtype),
                 size = log2.p.adjust),
             color = 'black',
             pch = 21) +
  ...
  labs(    x = expression(NES),
           y = expression(Oncogenic~Signatures),
           size = expression(-log[2]~(Adj.p-value)),
           fill = expression(Cell))
```


```{r}
# place order of CCA as subtype 1 and subtype 2 by rearranging column data
subtype.order <- c('K213','K214','K156','HUCCA1','RBE','D138','HuCCT1','TYBDC1','K055','D068','D131','TFK1','K100','YSCCC','SSP25')

mx.by.subtypes <- dcast(Sig, ID ~ factor(Cell,subtype.order), value.var = 'NES')

mx2.filter <- mx.by.subtypes[,colnames(mx.by.subtypes) %in% c(DRGROUP1)]

rownames(mx2.filter) <- mx.by.subtypes[,1]
mx2.filter <- as.matrix(mx2.filter) # this matrix will be used for PCA and tSNE
