...
b.out$Cell <- as.character(b.out$Cell)
...
Classifier.genes <- read.csv('ListOfClassifierGenes.csv', header = TRUE, stringsAsFactors = FALSE)

# write.csv(Expression.genes, file = 'ExpressionOfClassifierGenes.csv', row.names = FALSE)

Expression.genes <- read.csv('ExpressionOfClassifierGenes.csv', header = TRUE, stringsAsFactors = FALSE)
```
...
```{r}
setwd('~/CCA/module3_results')
pdf(paste0('BiomarkersIdentification_Fig5A_byPval_20200619.pdf'),
    paper = 'A4',
    width = 7, #inches
    height = 12, #inches
...
dat.CF <- Classifier.genes
dat.EX <- Expression.genes
...
myROC <- lapply(1:length(TestDrugs), function(n) {
  ...
  # from drug response table, we prepared CCA subtype table
  if(MyDrug == 'Subtype'){
  dat.Class <- drugSR %>% group_by(Cell,CCA.Subgroup) %>% summarise()
  ...
  # and prepared organ similarity table
  dat.Class <- drugSR %>% group_by(Cell,Organ.Subgroup) %>% summarise()
  dat.Class$Organ.Subgroup <- ifelse(dat.Class$Organ.Subgroup == 'liver-like','Sensitive','Resistant')
  colnames(dat.Class)[2] <- 'Class'
  } else {
  dat.Class <- drugSR %>% subset(Drug == MyDrug) %>% dplyr::select(Cell,Class.KDE)
  colnames(dat.Class)[2] <- 'Class'
  }
