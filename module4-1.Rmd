...
SigBioList <- read.csv('PredictedGenes_20191016.csv', header = TRUE, stringsAsFactors = FALSE)
...
```{r}
# create data for right, row annotation using coefficient values
mybar <- ppcos2
# modify Dim.1.2 sign
...
my5B <- apply(X = datin.apcc.sub,MARGIN = 1,FUN = scale) %>% t() %>% as.data.frame()

colnames(my5B) <- colnames(datin.apcc.sub)

# use unsupervised hierarhical clustering to classify cca patients
hr <- hclust(dist(t(my5B), method = 'euclidean',), method = 'ward.D2')
...
# rearrange genes in my5B
my5B1 <- my5B[rownames(mybar),ordertab$ID]
```
...
```{r}
# Plot heatmap
myFig5B <- ComplexHeatmap::Heatmap(as.matrix(my5B1), # my.dt.sorted
                  # column_km = 2,
                  # split = clusters,
                  # gap = unit(2,'mm'),
                  # row_gap = gap,
...
```{r}
# Plot heatmap
myFig5BCoef <- ComplexHeatmap::Heatmap(as.matrix(my5BCoef), # my.dt.sorted
                  # column_km = 2,
                  # split = clusters,
                  # gap = unit(2,'mm'),
                  # row_gap = gap,
...
setwd('C:/Users/patipark/Dropbox/CCA project/CCA RNA-seq/NewGeneSet_Module_InitialAnalysis/Figure_5A_files')
pdf(paste0('Fig5B_45Genes_Heatmap_JusakulCCA_20200402.pdf'),
    height = 7,
    width = 15)
draw(myFig5BCoef,
     height = unit(5,'in'),
...
my5BCoef4 <- my5BCoef %>% subset(!rownames(.) %in% c('TP53','AREG','TCOF1','TNFAIP2'))
...
my5BCoef5 <- my5BCoef4[mybarCoef5$Gene,]

# plot a heatmap
myFig5BCoef5 <- ComplexHeatmap::Heatmap(as.matrix(my5BCoef5), # my.dt.sorted
                  # column_km = 2,
                  # split = clusters,
                  # gap = unit(2,'mm'),
                  # row_gap = gap,
...
# y_auc <- known.class
patient.list <- colnames(my5BCoef5)
...
```{r}
# subset gene expression data from ~30% of the samples from my5BCoef
newX <- t(my5BCoef)
# random sampling
...
```{r}
#test set
demo.X <- t(my5BCoef) %>% as.data.frame() #as.matrix(datin.pca)
...
require(Rtsne)
set.seed(12345)
tsne_out <- Rtsne(as.matrix(t(my5BCoef)),
...
rownames(tsne.result) <- colnames(my5BCoef)
